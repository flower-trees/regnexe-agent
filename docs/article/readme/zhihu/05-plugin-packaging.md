# 插件该怎么加载，才能让开发和运维都不吵架？

先说结论：不要二选一，把"怎么打包"和"怎么加载"拆成两个独立维度，让每个角色用自己舒服的方式接入。这篇聊聊 Regnexe 怎么用四种方式同时满足开发和运维的诉求。

> 「Regnexe 实战系列」第 5 篇（共 10 篇），对应仓库 [`ExampleReadme05PluginPackagingTest`](https://github.com/flower-trees/regnexe-agent/blob/master/src/test/java/org/salt/regnexe/agent/core/example/readme/ExampleReadme05PluginPackagingTest.java)。上一篇：[04. @Plugin 注解打包一切](https://github.com/flower-trees/regnexe-agent/blob/master/docs/article/readme/zhihu/04-plugin-annotation.md)。
...
两边都没说错，只是关注点不一样——一个要的是类型安全和可测试性，一个要的是运维灵活度。强行二选一，最后通常是吵赢的一方推行自己的方案，另一方默默忍受。

Regnexe 没有让我们选边站，而是把"插件怎么打包"和"插件怎么加载"拆成了独立的维度，四种方式并存。

## 方式一：代码直打包——PluginDescriptor 是底层真相

前几篇用的 `withTool`/`withSkill`/`withSubAgent`/`@Plugin`，最终都会落到同一个东西上：一个装着若干 `CapabilityDescriptor` 的 `PluginDescriptor`。如果想完全手动控制，`PluginDescriptor.builder()` 直接开放了这条路：
...
一个 `PluginDescriptor`，三种能力类型，id 自动按 `pluginId + "." + name` 拼好。这是最"硬核"的方式，适合能力定义来自数据库、配置中心，需要在运行时动态拼装的场景。

> 这里有个坑值得单独提一下：三种能力共用一个 `pluginId` 时，Skill 的 `allowedTools` 必须写完整能力 id——`"trip-plugin.get_weather"`，不是裸的 `"get_weather"`。

## 方式二：包扫描——让插件"自己长出来"

如果插件类已经按 `@Plugin`/`@AgentSkill`/`@AgentSubAgent` 写好了，不需要一个个 `new` 出来注册，直接扫包：
...
它们不是互斥关系，一个应用里完全可以同时用——核心业务工具用 `@Plugin` 写死，运营临时配置的能力走目录加载。Regnexe 这套 harness 在这件事上的态度很明确：**不替你做选择，只把每个选项的成本都降到最低**。

---

你们团队的插件/配置管理，最后是怎么在"开发体验"和"运维灵活度"之间找平衡的？欢迎分享经验。

📌 上一篇：[04. @Plugin 注解打包一切](https://github.com/flower-trees/regnexe-agent/blob/master/docs/article/readme/zhihu/04-plugin-annotation.md) ｜ 下一篇：[06. 能力市场换成数据库意味着什么](https://github.com/flower-trees/regnexe-agent/blob/master/docs/article/readme/zhihu/06-marketplace.md)
