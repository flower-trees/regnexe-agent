# 子任务想用便宜模型？Sub-Agent 帮你把成本打下来 💰

> 「Regnexe 实战系列」第 3 篇（共 10 篇），对应仓库 [`ExampleReadme03SubAgentTest`](https://github.com/flower-trees/regnexe-agent/blob/master/src/test/java/org/salt/regnexe/agent/core/example/readme/ExampleReadme03SubAgentTest.java)。上一篇：[02. Skill](https://github.com/flower-trees/regnexe-agent/blob/master/docs/article/readme/juejin/02-skill.md)。

## 💸 一个很实际的成本问题

很多团队做多 Agent 系统会遇到这个问题：主任务需要强模型做复杂推理，但拆出来的某个子任务——比如"估算一下这次出差大概花多少钱"——逻辑很简单，用旗舰模型纯属浪费 token。

理想情况是：主 Agent 用强模型，子任务用更便宜的模型单独跑，互不影响。上一篇讲的 Skill 做不到这件事——它被设计成强制继承主模型。这篇要讲的 **Sub-Agent**，规则正好反过来。
...
`model` 默认值是 `"inherit"`——不配就跟 Skill 一样继承父模型；写了具体值，就走独立的 `ModelProvider` 分支单独构建模型实例。这个默认值我觉得挺贴心：默认安全，按需打开独立配置，不会因为忘了配置出现意外行为。
...
主 Agent 跑 `deepseek-v4-flash`，`expense_estimator` 内部跑 `aliyun:qwen-plus`，两个模型互不干扰，账单也分得清清楚楚。

## 🔐 关键点：ownTools 为什么外面真的看不到

`estimate_trip_cost` 这个工具，从头到尾没有出现在 `withTool` 或者 marketplace 的任何注册调用里，只存在于 `expenseEstimator.ownTools` 这一个地方。

这意味着主 Agent 的 Planner 在 Search 阶段，候选列表里压根不会出现 `estimate_trip_cost`，没法直接调用——唯一入口是先选中 `expense_estimator` 这个 Sub-Agent。这是真正意义上的"私有"，不是君子协定，是从能力市场的可见性上就把它隔离掉了。适合那些不想让外层 Agent 瞎调用、必须经过子任务封装处理的场景，比如涉及资金操作的工具。
...
- 要 → Sub-Agent
- 不要，就是想复用主 Agent 模型省成本 → Skill

两种类型不是谁更高级，是这套 harness 解决不同问题的工具。下一篇会讲怎么把这两种类型连同普通 tool 一起，用注解打包进一个类里，一次注册全搞定。

---

如果你也被"子任务模型成本"折磨过，欢迎评论区分享你的方案，点个赞再走 👍

📌 上一篇：[02. Skill](https://github.com/flower-trees/regnexe-agent/blob/master/docs/article/readme/juejin/02-skill.md) ｜ 下一篇：[04. @Plugin 注解打包一切](https://github.com/flower-trees/regnexe-agent/blob/master/docs/article/readme/juejin/04-plugin-annotation.md)
