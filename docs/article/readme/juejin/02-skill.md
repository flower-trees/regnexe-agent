# Agent 里的 Skill 和 Sub-Agent 怎么选？我踩过的坑都在这了

> 「Regnexe 实战系列」第 2 篇（共 10 篇），对应仓库 [`ExampleReadme02SkillTest`](https://github.com/flower-trees/regnexe-agent/blob/master/src/test/java/org/salt/regnexe/agent/core/example/readme/ExampleReadme02SkillTest.java)。上一篇：[01. withTool 极简接入](https://github.com/flower-trees/regnexe-agent/blob/master/docs/article/readme/juejin/01-multi-tool.md)。

## 🤔 一个会让人纠结很久的设计问题

做多 Agent 系统的时候，几乎每个人都会卡在这个问题上：

> "这个子能力，要不要给它配个独立的模型？要不要让它有自己的私有工具？"

配置太灵活，团队用起来会乱——有人配了模型有人没配，排查的时候根本搞不清哪个子能力在用哪个模型。配置太死板，又满足不了"有些子任务确实需要独立模型"的真实需求。

Regnexe 的处理方式：拆成 Skill 和 Sub-Agent 两种类型，规则定死，不留模糊空间。这篇先讲 Skill。

## 🔒 Skill 的铁律：只能继承，不能拥有

打开 `SkillConfig` 这个类，会发现一个挺有意思的事——它**压根没有 `model` 字段**。不是没暴露，是设计上就不给配：

> Skill 永远继承父 Agent 的模型，没有例外。

工具也一样，`SkillConfig` 只有 `allowedTools`（按 id 借用市场里已注册的工具），没有 `ownTools`（私有工具）。
...
这不是功能阉割，是刻意为之——Skill 就该是主 Agent 的一个延伸动作，不该自己另起炉灶。
...
⚠️ 注意顺序：`get_weather` 必须先用 `withTool` 注册进市场，`travel_advisor` 才能借到它。这是硬性依赖——`allowedTools` 里写的 id 在市场里找不到，Skill 内部就压根调不到这个工具，这个坑我自己也踩过一次。
...
这句话是写给 Planner（背后也是个 LLM）看的，相当于在候选能力列表里给这个 Skill 贴一张"什么时候该选我"的说明书。实测下来，description 里明确写触发条件，比单纯堆功能描述，命中率高出一个量级——这个技巧比 API 本身更值得记下来。
...
如果你的子任务需要**自己的模型**或者**私有工具**，那就不是 Skill 该管的事了——下一篇讲 Sub-Agent，规则正好反过来。Skill 和 Sub-Agent 都是这套 harness 暴露出来的能力类型，理解了边界，用起来会顺很多。

---

这篇有帮到你的话，欢迎点个赞 👍，下一篇见 Sub-Agent。

📌 上一篇：[01. withTool 极简接入](https://github.com/flower-trees/regnexe-agent/blob/master/docs/article/readme/juejin/01-multi-tool.md) ｜ 下一篇：[03. Sub-Agent：自己的模型自己的工具](https://github.com/flower-trees/regnexe-agent/blob/master/docs/article/readme/juejin/03-subagent.md)
