# 插件加载方式：开源框架 Regnexe 的四种姿势全解析

> 「Regnexe 实战系列」第 5 篇（共 10 篇），对应仓库 [`ExampleReadme05PluginPackagingTest`](https://github.com/flower-trees/regnexe-agent/blob/master/src/test/java/org/salt/regnexe/agent/core/example/readme/ExampleReadme05PluginPackagingTest.java)。上一篇：[04. 一个注解打包 4 种能力类型](https://github.com/flower-trees/regnexe-agent/blob/master/docs/article/readme/oschina/04-plugin-annotation.md)。

## 开发和运维的诉求经常不一致

做平台型项目常见的争论：工具应该写死在代码里，编译期就能查出问题；运维要能随时加减插件，不能每次都重新编译发版。两边都没说错，只是关注点不一样。Regnexe 没有强行二选一，而是把"插件怎么打包"和"插件怎么加载"拆成了独立的维度，四种方式并存。
...
> 注意：三种能力共用一个 `pluginId` 时，Skill 的 `allowedTools` 必须写完整能力 id——`"trip-plugin.get_weather"`，不是裸的 `"get_weather"`。

## 方式二：包扫描

插件类已经按 `@Plugin`/`@AgentSkill`/`@AgentSubAgent` 写好了，不需要一个个 `new` 出来注册，直接扫包：
...
如果你的项目也在开发灵活性和运维灵活度之间纠结，欢迎参考这套方案，去仓库给个 Star。

📌 上一篇：[04. 一个注解打包 4 种能力类型](https://github.com/flower-trees/regnexe-agent/blob/master/docs/article/readme/oschina/04-plugin-annotation.md) ｜ 下一篇：[06. 能力市场换成数据库](https://github.com/flower-trees/regnexe-agent/blob/master/docs/article/readme/oschina/06-marketplace.md)
